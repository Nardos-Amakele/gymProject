...
export async function getMyWorkoutPlans () {
  const res = await fetch("http://localhost:5000/api/members/8a9b5e1f-3522-45a1-b65b-b15b29ffaf75/getMyWorkouts", {cache:"no-store"});
  const data  = await res.json();
  const workoutPlans: WorkoutPlanType[] = data.data;
  return workoutPlans
}

export default async function MyWorkoutPlanPage() {
      ...
      <div className="bg-black text-white min-h-screen">
        ...
        <main className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 bg-[#252525] rounded-lg">
          {plans.map((plan, index) => (
            <Link key={index}  href={`myWorkoutPlan/${plan.id}`}>
